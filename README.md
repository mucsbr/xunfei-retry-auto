...
一个轻量 Go 反向代理：客户端先请求本服务，本服务再转发到讯飞 MaaS Anthropic 接口；当上游返回 `503` 且响应体能判断为 busy（例如 `error.code=10310` 或 message 包含 `busy` / `try again later`）时，会按配置自动重试。
...
| `MAX_RETRIES` | `1` | 命中 busy 503 后最多再重试的次数；`1` 表示最多请求上游 2 次 |
